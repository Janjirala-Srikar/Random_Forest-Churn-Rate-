...
def build_dashboard_data():
    ...
    validation_with_scores["RiskScore"] = pd.Series(
        y_val_prob,
        index=y_val.index
    )
    ...
    return {
        "kpis": {
            ...
            "active_rate": percent(
                raw_df["IsActiveMember"].mean()
            ),
            "avg_balance": round(
                float(raw_df["Balance"].mean()),
                2
            ),
            "avg_age": round(
                float(raw_df["Age"].mean()),
                1
            ),
            "avg_salary": round(
                float(raw_df["EstimatedSalary"].mean()),
                2
            ),
        },
        ...
        "segments": {
            ...
            "churn_by_geography":
                churn_by_segment("Geography"),

            "churn_by_gender":
                churn_by_segment("Gender"),

            "churn_by_products":
                churn_by_segment("NumOfProducts"),
        },

        "distributions": {
            "age": histogram(
                "Age",
                [18, 25, 35, 45, 55, 65, 75, 95]
            ),

            "credit_score": histogram(
                "CreditScore",
                [300, 450, 550, 650, 750, 850, 950]
            ),

            "balance": histogram(
                "Balance",
                [0, 25000, 75000, 125000,
                 175000, 225000, 275000]
            ),
        },
        ...
        "risk_bands": {
            "labels": [
                str(label)
                for label in risk_bands.index.tolist()
            ],

            "values": [
                int(value)
                for value in risk_bands.tolist()
            ],
        },
    }
...
print(
    f"Model ready - Accuracy: {ACCURACY} | "
